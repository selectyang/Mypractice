...
var _Waterfall = __webpack_require__(5);
...
new _GoTop2.default($('body'));
new _Waterfall2.default();
carousel($('.container'));

function carousel($ct) {
    $ct.each(function (index, node) {
        new _Carousel2.default($(node));
    });
}
...
Carousel.prototype.bind = function () {
	...
	this.$buttonPre.on('click', function (e) {
		e.preventDefault();
		if (_this.islock) _this.playPre(1);
	});
	this.$buttonNext.on('click', function (e) {
		e.preventDefault();
		if (_this.islock) _this.playNext(1);
	});
...
GoTop.prototype.bindEvent = function () {
	...
	$(window).on('scroll', function () {
		var nodeShow = $(window).scrollTop() > window.innerHeight;
		if (nodeShow) {
...
Waterfall.prototype.insetNode = function (ret) {
	...
	for (var i = 0; i < ret.length; i++) {
		//for循环中的load后会插入10张相同的第10张图片在页面上
		var nodeTemps = '';
...
Waterfall.prototype.render = function () {
	...
	nodeNum = parseInt($('.portfolio .card1-list').width() / nodeWidth), nodeSumHeight = [];
